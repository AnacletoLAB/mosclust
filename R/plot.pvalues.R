...
function(l,alpha=1e-02,legendy=0, leg_label=NULL, colors=TRUE) { 
  ...
  plot(1:n.clusterings, l[[1]]$p.value,xlim=c(1,n.clusterings), ylim=c(miny,2), xlab="clusterings", 
	...
	axis(1, at=1:n.clusterings, labels=xlab);
	axis(2);
	#if (n.test>1)
	if (colors == TRUE) 
	  for (i in 1:n.test) {
	    lines(1:n.clusterings,l[[i]]$p.value,lty=lpat[i], type="o", col=i) 
	    # text(l$x[1],l$y[1],pos=1,labels=lab[i]);
		}
	else 
	  for (i in 1:n.test) {
	    lines(1:n.clusterings,l[[i]]$p.value,lty=lpat[i], type="o"); 
	    # text(l$x[1],l$y[1],pos=1,labels=lab[i]);
	  }	 
	lines(1:n.clusterings,rep(alpha,n.clusterings),lty=2, type="l");
	text(ceiling(n.clusterings/2),alpha,labels=paste("alpha=", alpha));
	# making legend
	if (is.null(leg_label))
	   leg_label <- paste("test ",1:n.test,sep="");
	if (legendy!=0)
	  if (colors == TRUE)
	    legend(1,legendy, legend=leg_label,lty=lpat[1:n.test], col=1:n.test)
		else
		  legend(1,legendy, legend=leg_label,lty=lpat[1:n.test]);
}
